...
int main() {
    std::ofstream df1("Saunuoliai.txt");    //išvalo failus
    ...
    char gen, test, zodis, ne_raide, atsakymas, tavo_zodis;
...
std::cout << "Ar norite atlikti veiksmo greicio analize(naudojant vector)? t - taip, n - ne" << std::endl;
std::cin >> atsakymas;
if (atsakymas == 't') {
...
std::cout << "ar norime sugeneruot sabloninius failus v0.2 versijai? t - generuosime, n - ne" << std::endl; // jei norim, sukuriam šabloninius failus norimo dydžio mok kiekio ir nd kiekio
std::cin >> test;
if (test == 't') {
    ...
    for (int i = 0; i < f_kiekis; i++) {
        ...
        generavimas(v2mok_kiekis, v2ND_kiekis, pavadinimai[i]);         //generuoju nurodytą kiekį failų su nurodytais kiekiais mokinių ir pažymių
    }
...
std::cin >> zodis;
if (zodis == 't') f_trinimas(pavadinimai);
...
if (gen == 't') {
    ...
    while (true) {
        try {           //tikrinu ar įvestas skaičius
            std::cout << "Kiek ND buvo uzduota?" << std::endl;
            std::cin >> ND_kiekis;
            if (std::cin.fail()) {          //jei įvesta vertė neatitinka iš anksto nustatyto kintamojo duomenu tipo, įsijungia if funkcija

                std::cin.clear();           // ši eilute ištrina errora kylanti iš neteisingos vertes įvedimo
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');         // įvesta verte ištrinama
                throw 500;      //
            }
            break;          // jei vertė teisinga, tai ciklas nutraukiamas ir programa gali testis
        }
        catch (int a) {
            std::cout << "Prasau iveskite skaiciu. Bandom vel" << std::endl;        // išmetama žinute, jog įvestas ne skaičius
        }
    }
...
isvedimas(mok);  // išvedimo funkcija
...
int failo_ilgis() {
    ...
    while (std::getline(df, temp)) {  // skaičiuoja eilučių kiekį
        x++;
    }
    x = x - 1;
    df.close();
...
void nuskaitymas(std::vector<mokinys>& strukturos_pav, int ilgis, int ND_kiekis, std::string failas) { // funkcijoj sukuriu naują mokinys struktura ir nuskaičius viska, gražiai viską įmetu į mokinys mok strukturą
    std::string temp;
    char raid[50];  //laikinas char, skirtas vardo ir pavardes priskyrimui

    std::ifstream df(failas);
    std::getline(df, temp);  //Praleidžiu nereikalinga pirmaja eilute

    for (int i = 0; i < ilgis; i++) {
        mokinys current;
        df.get(raid, 20);  //Perskaitau ir priskyriu vardą
        current.Vardas = raid;
        df.get(raid, 26);  //perskaitau ir priskyriu pavardę
        current.Pavarde = raid;

        for (int j = 0; j < ND_kiekis; j++) {  //perskaitau ir priskyriu pažymius
            int Pazymys_temp;
            df >> Pazymys_temp;
            current.Pazymys.push_back(Pazymys_temp);
        }
        df >> current.Egz;
        current.galutinis = galutinis(vidurkis(current), current.Egz);          // priskiriu galutinio balo vertę
        std::getline(df, temp);  // praleidziu likusia eilutę
        strukturos_pav.push_back(current);
    }
    df.close();
}
void space_remove(mokinys& mok) {  // funkcija skirta panaikinti nereikalingus tarpus. just in case
...
double vidurkis(const mokinys& mok) {
    if (mok.Pazymys.empty()) return 0.0;//tikrina ar yra pažymiu.   .empty() gražina bool tipą

    double sum = std::accumulate(mok.Pazymys.begin(), mok.Pazymys.end(), 0);
    return sum / mok.Pazymys.size();
}
double mediana(mokinys& mok) {
    if (mok.Pazymys.empty()) return 0.00;  //tikrina ar yra pažymiu.   .empty() gražina bool tipą

    std::sort(mok.Pazymys.begin(), mok.Pazymys.end());  //sortina pažymiu vektorių didejimo tvarka

    int size = mok.Pazymys.size();         //jei lyginis paž skaicius, ima dvieju viduriniu verčiu vidurkį
    if (size % 2 == 0) {
        return (mok.Pazymys[size / 2 - 1] + mok.Pazymys[size / 2]) / 2;
    }
    else {                                  // jei nelyginis skaičius, tai išveda vidurinę vertę
...
void generavimas(int mok_kiekis, int ND_kiekis, std::string pavadinimas) {
    ...
    df << "Vardas                   Pavarde                    ";       // į antrastę(pirma eilute) įvedu vardą ir pavardę
    for (int i = 1; i <= ND_kiekis; i++) {
        std::string Hnd = "ND" + std::to_string(i);     //tą patį darau su ND
    ...
    for (int i = 1; i <= mok_kiekis; i++) {     //įrasau vardo ir pavardės šabloną
        ...
        for (int j = 1; j <= ND_kiekis + 1; j++) {  // kiekvienam mokyniui įrasau iš anksto nustatytą, atsitiktinį nd pažymį + egz.
            int tempND;
            while (true) {
                tempND = rand() % 11;
                if (tempND != 0) {
                    break;              // kadangi mokytojai dazniausiai neskiria 0 kaip įvertinimo, tai gavus atsitiktinį nulio ivertinimą, bandom vel
                }
            }

            if (tempND == 10) {
                df << tempND << spacing(std::to_string(tempND), 11);        // palieku tvarkingą space kieki gavus dešimtuką
            }
            else {
...
void f_trinimas(std::vector<std::string>& pavadinimai) {
    for (int i = 0; i < pavadinimai.size(); i++) {
        remove(pavadinimai[i].c_str());     //kadangi komanda "remove" reikalauja char tipo, tai pradžioj mano string tipo kintamaji - pavadinimai, verčiu į char array
    }
}
void gal_isvedimas(std::vector<mokinys>& gal_mok) { //tiesiog išveda vardą, pavardę, galutinį pazymį ir pavadinimą šaunuoliai/begedžiai
    int kiekis;
    char vid_med;

    for (auto& m : gal_mok) {       //panaikina visus nuskaitytus tarpus, nes vėliau galetų kilt prblemų su spacing funkcija
        space_remove(m);
    }
...
void isskirstymas(std::vector<mokinys>& pavadinimas, int mok_kiekis) {      //atidaro failus ir jei galutinis balas nemažesnis uz 5, įraso mokinį į šaunuoliai.txt
...
void l_nuskaitymas(std::list<studentas>& std, int ilgis, int ND_kiekis, std::string failas) {
    ...
    std::ifstream df(failas);
    std::getline(df, temp);  //Praleidžiu nereikalingą pirmają eilutę
    ...
    for (int i = 0; i < ilgis; i++) {
        ...
        df >> current.Egz;
        current.galutinis = galutinis(l_vidurkis(current), current.Egz);          // priskiriu galutinio balo vertę
        std::getline(df, temp);  // praleidziu likusia eilutę
...
void l_isskirstymas(std::list<studentas>& std, int mok_kiekis) {      //atidaro failus ir jei, galutinis balas nemažesnis uz 5, įraso mokinį į šaunuoliai.txt
